 ...
 * (Command) from a menu. The window menu has two parts: One contains the menu
 * buttons associated with the menu. The other part is used to print the results
 * of the selected command. There may be a bug if a command writes a result and
 * it has the last command property. In this case, the window with the written
 * result is removed too fast.
 */
class SwingDialogMenu extends JDialog implements ActionListener {
  ...
  // Renders the selected output. Title refers to the title of the command
  ...
  private void executeOption(int option) {
    if (option != 0) {
      try {
        ...
        // updateOutput(_menu.title(), _menu.entry(option - 1).title() + ": " + e);
        // //$NON-NLS-1$
    ...
    if (option == 0 || _menu.entry(option - 1).isLast()) {
      clearAndHide();
      System.out.printf(" clear and hide for menu %s\n", _menu.title());
    } else
      setVisible(true);
